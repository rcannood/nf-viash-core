...
meta = [
  ...
  "build_info" : {
    "config" : "/home/rcannood/workspace/viash-io/viash/src/test/resources/test_languages/./csharp/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "native|docker",
    "output" : "/home/rcannood/workspace/viash-io/nf-viash/viash-target/nextflow/test_languages_csharp",
    "viash_version" : "0.10.0-dev",
    "git_commit" : "9b574b1f4f0043f84fee9cc9ac5b7df95d6c34a6",
    "git_remote" : "git@github.com:viash-io/viash.git",
    "git_tag" : "0.9.4-162-g9b574b1f"
  },
  "package_config" : {
    "source" : ".",
...
var par = new {
  input = @"${VIASH_PAR_INPUT//\\"/\\"\\"}",
  real_number = Convert.ToDouble(@"${VIASH_PAR_REAL_NUMBER//\\"/\\"\\"}"),
  whole_number = Convert.ToInt32(@"${VIASH_PAR_WHOLE_NUMBER//\\"/\\"\\"}"),
  long_number = $( if [ ! -z ${VIASH_PAR_LONG_NUMBER+x} ]; then echo "Convert.ToInt64(@"'"'"${VIASH_PAR_LONG_NUMBER//\\"/\\"\\"}"'"'")"; else echo "(long?) null"; fi ),
  s = @"${VIASH_PAR_S//\\"/\\"\\"}",
  truth = $( if [ ! -z ${VIASH_PAR_TRUTH+x} ]; then echo "bool.Parse(@"'"'"${VIASH_PAR_TRUTH//\\"/\\"\\"}"'"'".ToLower())"; else echo "(bool?) null"; fi ),
  falsehood = $( if [ ! -z ${VIASH_PAR_FALSEHOOD+x} ]; then echo "bool.Parse(@"'"'"${VIASH_PAR_FALSEHOOD//\\"/\\"\\"}"'"'".ToLower())"; else echo "(bool?) null"; fi ),
  reality = $( if [ ! -z ${VIASH_PAR_REALITY+x} ]; then echo "bool.Parse(@"'"'"${VIASH_PAR_REALITY//\\"/\\"\\"}"'"'".ToLower())"; else echo "(bool?) null"; fi ),
  output = $( if [ ! -z ${VIASH_PAR_OUTPUT+x} ]; then echo "@"'"'"${VIASH_PAR_OUTPUT//\\"/\\"\\"}"'"'""; else echo "(string) null"; fi ),
  log = $( if [ ! -z ${VIASH_PAR_LOG+x} ]; then echo "@"'"'"${VIASH_PAR_LOG//\\"/\\"\\"}"'"'""; else echo "(string) null"; fi ),
  optional = $( if [ ! -z ${VIASH_PAR_OPTIONAL+x} ]; then echo "@"'"'"${VIASH_PAR_OPTIONAL//\\"/\\"\\"}"'"'""; else echo "(string) null"; fi ),
  optional_with_default = $( if [ ! -z ${VIASH_PAR_OPTIONAL_WITH_DEFAULT+x} ]; then echo "@"'"'"${VIASH_PAR_OPTIONAL_WITH_DEFAULT//\\"/\\"\\"}"'"'""; else echo "(string) null"; fi ),
  multiple = $( if [ ! -z ${VIASH_PAR_MULTIPLE+x} ]; then echo "@"'"'"${VIASH_PAR_MULTIPLE//\\"/\\"\\"}"'"'".Split("'"'";"'"'").ToArray()"; else echo "(string) null"; fi ),
  multiple_pos = $( if [ ! -z ${VIASH_PAR_MULTIPLE_POS+x} ]; then echo "@"'"'"${VIASH_PAR_MULTIPLE_POS//\\"/\\"\\"}"'"'".Split("'"'";"'"'").ToArray()"; else echo "(string) null"; fi )
};
var meta = new {
  name = @"${VIASH_META_NAME//\\"/\\"\\"}",
  resources_dir = @"${VIASH_META_RESOURCES_DIR//\\"/\\"\\"}",
  executable = @"${VIASH_META_EXECUTABLE//\\"/\\"\\"}",
  config = @"${VIASH_META_CONFIG//\\"/\\"\\"}",
  temp_dir = @"${VIASH_META_TEMP_DIR//\\"/\\"\\"}",
  cpus = $( if [ ! -z ${VIASH_META_CPUS+x} ]; then echo "Convert.ToInt32(@"'"'"${VIASH_META_CPUS//\\"/\\"\\"}"'"'")"; else echo "(int?) null"; fi ),
  memory_b = $( if [ ! -z ${VIASH_META_MEMORY_B+x} ]; then echo "Convert.ToInt64(@"'"'"${VIASH_META_MEMORY_B//\\"/\\"\\"}"'"'")"; else echo "(long?) null"; fi ),
  memory_kb = $( if [ ! -z ${VIASH_META_MEMORY_KB+x} ]; then echo "Convert.ToInt64(@"'"'"${VIASH_META_MEMORY_KB//\\"/\\"\\"}"'"'")"; else echo "(long?) null"; fi ),
  memory_mb = $( if [ ! -z ${VIASH_META_MEMORY_MB+x} ]; then echo "Convert.ToInt64(@"'"'"${VIASH_META_MEMORY_MB//\\"/\\"\\"}"'"'")"; else echo "(long?) null"; fi ),
  memory_gb = $( if [ ! -z ${VIASH_META_MEMORY_GB+x} ]; then echo "Convert.ToInt64(@"'"'"${VIASH_META_MEMORY_GB//\\"/\\"\\"}"'"'")"; else echo "(long?) null"; fi ),
  memory_tb = $( if [ ! -z ${VIASH_META_MEMORY_TB+x} ]; then echo "Convert.ToInt64(@"'"'"${VIASH_META_MEMORY_TB//\\"/\\"\\"}"'"'")"; else echo "(long?) null"; fi ),
  memory_pb = $( if [ ! -z ${VIASH_META_MEMORY_PB+x} ]; then echo "Convert.ToInt64(@"'"'"${VIASH_META_MEMORY_PB//\\"/\\"\\"}"'"'")"; else echo "(long?) null"; fi ),
  memory_kib = $( if [ ! -z ${VIASH_META_MEMORY_KIB+x} ]; then echo "Convert.ToInt64(@"'"'"${VIASH_META_MEMORY_KIB//\\"/\\"\\"}"'"'")"; else echo "(long?) null"; fi ),
  memory_mib = $( if [ ! -z ${VIASH_META_MEMORY_MIB+x} ]; then echo "Convert.ToInt64(@"'"'"${VIASH_META_MEMORY_MIB//\\"/\\"\\"}"'"'")"; else echo "(long?) null"; fi ),
  memory_gib = $( if [ ! -z ${VIASH_META_MEMORY_GIB+x} ]; then echo "Convert.ToInt64(@"'"'"${VIASH_META_MEMORY_GIB//\\"/\\"\\"}"'"'")"; else echo "(long?) null"; fi ),
  memory_tib = $( if [ ! -z ${VIASH_META_MEMORY_TIB+x} ]; then echo "Convert.ToInt64(@"'"'"${VIASH_META_MEMORY_TIB//\\"/\\"\\"}"'"'")"; else echo "(long?) null"; fi ),
  memory_pib = $( if [ ! -z ${VIASH_META_MEMORY_PIB+x} ]; then echo "Convert.ToInt64(@"'"'"${VIASH_META_MEMORY_PIB//\\"/\\"\\"}"'"'")"; else echo "(long?) null"; fi )
};
var dep = new {
