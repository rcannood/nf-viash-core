...
meta = [
  ...
  "build_info" : {
    "config" : "/home/rcannood/workspace/viash-io/viash/src/test/resources/test_languages/./bash/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "native|docker|throwawayimage",
    "output" : "/home/rcannood/workspace/viash-io/nf-viash/viash-target/nextflow/test_languages_bash",
...
$( if [ ! -z ${VIASH_PAR_REAL_NUMBER+x} ]; then echo "${VIASH_PAR_REAL_NUMBER}" | sed "s#'#'\\"'\\"'#g;s#.*#par_real_number='&'#" ; else echo "# par_real_number="; fi )
$( if [ ! -z ${VIASH_PAR_WHOLE_NUMBER+x} ]; then echo "${VIASH_PAR_WHOLE_NUMBER}" | sed "s#'#'\\"'\\"'#g;s#.*#par_whole_number='&'#" ; else echo "# par_whole_number="; fi )
$( if [ ! -z ${VIASH_PAR_LONG_NUMBER+x} ]; then echo "${VIASH_PAR_LONG_NUMBER}" | sed "s#'#'\\"'\\"'#g;s#.*#par_long_number='&'#" ; else echo "# par_long_number="; fi )
$( if [ ! -z ${VIASH_PAR_S+x} ]; then echo "${VIASH_PAR_S}" | sed "s#'#'\\"'\\"'#g;s#.*#par_s='&'#" ; else echo "# par_s="; fi )
$( if [ ! -z ${VIASH_PAR_TRUTH+x} ]; then echo "${VIASH_PAR_TRUTH}" | sed "s#'#'\\"'\\"'#g;s#.*#par_truth='&'#" ; else echo "# par_truth="; fi )
$( if [ ! -z ${VIASH_PAR_FALSEHOOD+x} ]; then echo "${VIASH_PAR_FALSEHOOD}" | sed "s#'#'\\"'\\"'#g;s#.*#par_falsehood='&'#" ; else echo "# par_falsehood="; fi )
$( if [ ! -z ${VIASH_PAR_REALITY+x} ]; then echo "${VIASH_PAR_REALITY}" | sed "s#'#'\\"'\\"'#g;s#.*#par_reality='&'#" ; else echo "# par_reality="; fi )
...
$( if [ ! -z ${VIASH_PAR_OPTIONAL_WITH_DEFAULT+x} ]; then echo "${VIASH_PAR_OPTIONAL_WITH_DEFAULT}" | sed "s#'#'\\"'\\"'#g;s#.*#par_optional_with_default='&'#" ; else echo "# par_optional_with_default="; fi )
$( if [ ! -z ${VIASH_PAR_MULTIPLE+x} ]; then echo "${VIASH_PAR_MULTIPLE}" | sed "s#'#'\\"'\\"'#g;s#.*#par_multiple='&'#" ; else echo "# par_multiple="; fi )
$( if [ ! -z ${VIASH_PAR_MULTIPLE_POS+x} ]; then echo "${VIASH_PAR_MULTIPLE_POS}" | sed "s#'#'\\"'\\"'#g;s#.*#par_multiple_pos='&'#" ; else echo "# par_multiple_pos="; fi )
