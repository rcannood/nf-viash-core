...
meta = [
  ...
  "build_info" : {
    "config" : "/home/rcannood/workspace/viash-io/viash/src/test/resources/test_languages/./r/script.vsh.R",
    "runner" : "nextflow",
    "engine" : "native|docker",
    "output" : "/home/rcannood/workspace/viash-io/nf-viash/viash-target/nextflow/test_languages_r",
...
par <- list(
  ...
  "real_number" = $( if [ ! -z ${VIASH_PAR_REAL_NUMBER+x} ]; then echo -n "as.numeric('"; echo -n "$VIASH_PAR_REAL_NUMBER" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "')"; else echo NULL; fi ),
  "whole_number" = $( if [ ! -z ${VIASH_PAR_WHOLE_NUMBER+x} ]; then echo -n "as.integer('"; echo -n "$VIASH_PAR_WHOLE_NUMBER" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "')"; else echo NULL; fi ),
  "long_number" = $( if [ ! -z ${VIASH_PAR_LONG_NUMBER+x} ]; then echo -n "bit64::as.integer64('"; echo -n "$VIASH_PAR_LONG_NUMBER" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "')"; else echo NULL; fi ),
  "s" = $( if [ ! -z ${VIASH_PAR_S+x} ]; then echo -n "'"; echo -n "$VIASH_PAR_S" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'"; else echo NULL; fi ),
  "truth" = $( if [ ! -z ${VIASH_PAR_TRUTH+x} ]; then echo -n "as.logical(toupper('"; echo -n "$VIASH_PAR_TRUTH" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'))"; else echo NULL; fi ),
  "falsehood" = $( if [ ! -z ${VIASH_PAR_FALSEHOOD+x} ]; then echo -n "as.logical(toupper('"; echo -n "$VIASH_PAR_FALSEHOOD" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'))"; else echo NULL; fi ),
  "reality" = $( if [ ! -z ${VIASH_PAR_REALITY+x} ]; then echo -n "as.logical(toupper('"; echo -n "$VIASH_PAR_REALITY" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'))"; else echo NULL; fi ),
  "output" = $( if [ ! -z ${VIASH_PAR_OUTPUT+x} ]; then echo -n "'"; echo -n "$VIASH_PAR_OUTPUT" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'"; else echo NULL; fi ),
  "log" = $( if [ ! -z ${VIASH_PAR_LOG+x} ]; then echo -n "'"; echo -n "$VIASH_PAR_LOG" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'"; else echo NULL; fi ),
  "optional" = $( if [ ! -z ${VIASH_PAR_OPTIONAL+x} ]; then echo -n "'"; echo -n "$VIASH_PAR_OPTIONAL" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'"; else echo NULL; fi ),
  "optional_with_default" = $( if [ ! -z ${VIASH_PAR_OPTIONAL_WITH_DEFAULT+x} ]; then echo -n "'"; echo -n "$VIASH_PAR_OPTIONAL_WITH_DEFAULT" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "'"; else echo NULL; fi ),
  "multiple" = $( if [ ! -z ${VIASH_PAR_MULTIPLE+x} ]; then echo -n "strsplit('"; echo -n "$VIASH_PAR_MULTIPLE" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "', split = ';')[[1]]"; else echo NULL; fi ),
  "multiple_pos" = $( if [ ! -z ${VIASH_PAR_MULTIPLE_POS+x} ]; then echo -n "strsplit('"; echo -n "$VIASH_PAR_MULTIPLE_POS" | sed "s#['\\\\]#\\\\\\\\&#g"; echo "', split = ';')[[1]]"; else echo NULL; fi )
)
meta <- list(
